...
const WishlistPage = () => {
  ...
  const handleRemoveProduct = async (wishlistId, productId) => {
    try {
      ...
      const updatedWishlists = wishlists.map((wishlist) => ({
        ...wishlist,
        products: wishlist.products.filter((product) => product.productId !== productId),
      }));
  ...
  const handleDeleteWishlist = async (wishlistId) => {
    try {
      ...
      const updatedWishlists = wishlists.filter((wishlist) => wishlist.wishlistId !== wishlistId);
      setWishlists(updatedWishlists);
    } catch (error) {
      ...
      <Container maxWidth="md">
        <Grid container justifyContent="center" alignItems="center" spacing={2}>
          ...
          <IconButton onClick={toggleFormVisibility}>
            <CreateIcon />
            <Typography variant="body2" style={{ marginLeft: '8px' }}>New Wishlist</Typography>
            </IconButton>
          </Grid>
