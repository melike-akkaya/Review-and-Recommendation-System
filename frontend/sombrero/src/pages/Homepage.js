...
import { getProductById, getLastFourProducts } from '../services/ProductService'; // Adjust the path as necessary
    ...
    <div>
      ...
      <Box display="flex" justifyContent="center" alignItems="center">
        ...
        {topProducts.map((product, index) => {
          ...
          return (
            <Link href={`/product/${productId}`} key={productId} sx={{ textDecoration: 'none' }}>
              <Card sx={{ width: 195, padding: 2, textAlign: 'center', margin: '0 10px' }}>
                <CardContent>
                  <img src={imageURL} alt={name} style={{ width: '100%' }} />
                  <Typography variant="h6" gutterBottom>{name}</Typography>
                  <Typography variant="body2" color="textSecondary" gutterBottom>${price.toFixed(2)}</Typography>
                </CardContent>
              </Card>
            </Link>
          );
      ...
      <Box display="flex" justifyContent="center" alignItems="center">
        ...
        {newProducts.slice(currentProductIndexNew, currentProductIndexNew + 3).map((product) => {
