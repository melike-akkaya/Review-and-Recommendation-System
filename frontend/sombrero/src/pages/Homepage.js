...
const newProducts = [
  ...
  },
  {
    ...
    productPrice: "$45",
    description: "Description for New Product 4",
  },
];
...
const Homepage = () => {
  ...
  const handleNextNew = () => {
    setCurrentProductIndexNew(
      (prevIndex) => (prevIndex + 1) % newProducts.length
    );
  };
    ...
    <div>
      ...
      <Box display="flex" justifyContent="center" alignItems="center">
        ...
        {[0, 1, 2, 3].map((index) => {
          ...
          const { id, imageURL, productName, productPrice } =
            topProducts[productIndex];
          return (
            <Link
              href={`/product/${id}`}
              key={productIndex}
      ...
      <Box display="flex" justifyContent="center" alignItems="center">
        ...
        {[0, 1, 2].map((index) => {
          const productIndex =
            (currentProductIndexNew + index) % newProducts.length;
          const { id, imageURL, productName, productPrice, description } =
